...
By default the proxy listens on port `8082`. If that port is already in use on your machine you can run the proxy on an alternate port (example: `8083`).
...
This helper activates the local venv (if present), forces the project `.env` to be loaded, and starts `uvicorn` on port `8083` to avoid common port conflicts.

Manual start (choose a free port):

```bash
# start on 8083 instead of 8082
...
PowerShell (adjust port if you started the proxy on a different port, e.g. `8083`):

```powershell
$env:ANTHROPIC_AUTH_TOKEN="freecc"; $env:ANTHROPIC_BASE_URL="http://localhost:8083"; claude
```

Bash (adjust port if you started the proxy on a different port, e.g. `8083`):
...
-- `ANTHROPIC_BASE_URL` is `http://localhost:8082` (or the port you started the proxy on, e.g. `http://localhost:8083`), not `http://localhost:8082/v1`.
...
Claude Code CLI / IDE
        ...
        v
Free Claude Code proxy (:8083 by default in this repo) or the port you choose
        |
